...
public record NeoForgeWindow(Window window) implements MinecraftWindow {
    ...
    public MinecraftMonitor findBestMonitor() {
        return new NeoForgeMonitor(window.findBestMonitor());
    }
    ...
    public Optional<MinecraftVideoMode> getPreferredFullscreenVideoMode() {
        return window.getPreferredFullscreenVideoMode().map(NeoForgeVideoMode::new);
    }
